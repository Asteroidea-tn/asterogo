...
func (f FileWriterWithLevel) WriteLevel(level zerolog.Level, p []byte, formattedLog bool) (int, error) {
	if !formattedLog {
		_, err := f.Logger.Write(p)
		return len(p), err
	}
...
func collectExtraFields(entry map[string]interface{}) []string {
	standard := map[string]bool{"time": true, "message": true, "level": true, "caller": true}
	var extras []string
	for k, v := range entry {
		if !standard[k] {
...
func deleteOldLogFiles(logDir string, maxFiles int) error {
	...
	for _, file := range logFiles[:max(0, len(logFiles)-maxFiles)] {
...
func InitLogger(cfg CofigLogger) {
	...
	zerolog.TimestampFunc = func() time.Time { return time.Now().Local() }
...
func buildFileWriter(cfg CofigLogger) *FileWriterWithLevel {
	...
	filename := fmt.Sprintf("%s_%s%s.log", cfg.LogFileName, time.Now().Format("02-01-2006_15-04-05"), suffix)
